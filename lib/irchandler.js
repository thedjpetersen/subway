...
var irchandler = exports.irchandler = function(socket) {
  ...
  socket.on('connect', function(data) {
    ...
    socket.on('join', function(name) {
      client.join(name);
      if(current_user){
        if(!logger_users.hasOwnProperty(name.toLowerCase())){
          logger_users[name.toLowerCase()] = current_user.username;
        }
      }
    });
    ...
    socket.on('part', function(name) {
      client.part(name);
      if(current_user){
        if(logger_users[name.toLowerCase()] == current_user.username){
          delete logger_users[name.toLowerCase()];
          clients.forEach(function(client) {
            if(client.chans.indexOf(name.toLowerCase()) !== -1){
              logger_users[name.toLowerCase()] = client.nick;
            }
          });
        }
      }
    ...
    socket.on('say', function(data) {
      ...
      if(logger_users[data.target.toLowerCase()] == current_user.username) {
    ...
    var activateListener = function(event, argNames) {
      ...
      client.addListener(event, function() {
        ...
        if(event == 'message'){
          if(logger_users[args.to.toLowerCase()] == current_user.username) {
            log_message(args.target, {user: args.from, message: args.text});
          }
        }
      });
    };
