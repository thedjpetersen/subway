...
const get_plugin = require("./plugins").get_plugin;
...
const handle_irc = require("../src/js/handle_irc");
...
const logMessage = function(server, channel, attributes, username) {
  ...
  let target = loggers[server][channel];

  // If it is a private message
  if(channel.indexOf("#") === -1) {
    const targetIs = [channel, attributes.from].sort().join("#");
  ...
  const save_obj = _.extend(attributes, {server: server, to: channel});
  save_obj.attributes = JSON.stringify(_.omit(attributes, "message", "from", "server", "to", "type", "text"));

  if (target === username) {
    Message.create(save_obj);
  }
};

const attach_listener = function(client, socket, backbone_models) {
...
const restore_connection = function(user, io, req) {
  const backbone_models = _.clone(require("../src/js/models/models"));

  // See if our user has an active connection
  const has_connection = _.has(connections, user.username);

  // Find the socket associated with the request
  const socket = _.find(io.sockets.sockets, function(sock) {
    return sock.id === req.body.socketid;
  });
  ...
  if (typeof socket !== "undefined" && has_connection) {
    ...
    if(typeof connections[user.username].irc_conn !== "undefined") {
      // Set our irc connection to existing connection
      socket.irc_conn = connections[user.username].irc_conn;
    } else {
      // Store our new irc connection
...
const connection = function(io, app) {
  ...
  // Setup our debug endpoint
  if (typeof argv.debug !== "undefined") {
    debug = io.of('/debug');
    require("./debug")(app, debug, connections);
  }

  io.sockets.on("connection", function (socket) {
    socket.clients = {};

    const add_client = function(data) {
      const backbone_models = require("../src/js/models/models");

      const connect_data = _.extend({}, {
        userName: "subway",
        channels: [],
        debug: true,
        retryCount: 1,
      ...
      let client = new irc.Client(data.server, data.nick, connect_data);

      client.on("abort", function() {
        // Remove connection from our local models
        if (socket.irc_conn) {
          socket.irc_conn.get("connections").remove(data.server);
        }

        // Clean up client reference
        client = null;
        socket.emit("connection_error", data);
        socket.emit("connection_removed", {connection: data.server});

        // Remove connection from local models
        if (socket.irc_conn) {
          socket.irc_conn.get("connections").remove(data.server);
        }
      });
      ...
      if(socket.irc_conn) {
        backbone_models.username = socket.user.username;
        backbone_models.logMessage = logMessage;
        backbone_models.irc = socket.irc_conn;
      }
    ...
    // we iterate through their active connections and close
    // them
    ...
    socket.on("say", function(data) {
      const client = socket.clients[data.server];

      if (typeof socket.irc_conn !== "undefined") {
        const server = socket.irc_conn.get("connections").get(data.server);
    ...
    socket.on("command", function(data) {
      ...
      const includeChannel = function(args) {
        if(args.length > 1) {
          if (args[1].indexOf("#") !== 0) {
      ...
      switch (args[0].toLowerCase()) {
        ...
        case "msg":
          if (typeof socket.irc_conn !== "undefined") {
            const target_server = socket.irc_conn.get("connections")
    ...
    socket.on("saveSettings", function(data) {
      if(socket.user) {
        Settings.findOne({where: {user_id: socket.user.id}}, function(err, settings) {
          const new_settings = {user_id: socket.user.id, settings: JSON.stringify(data)};
          if(settings) {
