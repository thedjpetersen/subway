...
var restore_connection = function(user, io, req) {
  ...
  if (socket !== undefined && has_connection) {
    socket.clients = connections[req.body.username];
    var user_id = user.user_id;
    Connection.findOne({where: {user_id: user_id}}, function(err, connection) {
      if (connection && socket.logged_in && !_.isEmpty(socket.clients) ) {
        socket.emit("restore_connection", connection.connection_data);
      }
    });
  } else {
    connections[req.body.username] = socket.clients;
  }
...
var connection = function(io, app) {
  ...
  io.sockets.on("connection", function (socket) {
    ...
    socket.on("sync_connections", function(data) {
      if(socket.logged_in) {
        Connection.findOne({where: {user_id: socket.user.user_id}}, function(err, connection) {
          if(!connection) {
            connection = new Connection({
              user_id: socket.user.user_id,
              connection_data: JSON.stringify(data)
            });
          } else {
            connection.connection_data = JSON.stringify(data);
          }
          connection.save();
        });
      }
