        ...
        instance.client.addListener(event, function() {
          ...
          switch (event) {
            case 'connect':
              ...
              break;
            case 'join':
              if(instance.client.chans[args.channel.toLowerCase()] !== undefined) {
                instance.client.chans[args.channel.toLowerCase()].unread_messages = 0;
                instance.client.chans[args.channel.toLowerCase()].unread_mentions = 0;
              }
            ...
            case 'message#':
              // update unreads
              try {
                if (instance.client.chans[args.to.toLowerCase()].unread_messages === undefined) {
                  instance.client.chans[args.to.toLowerCase()].unread_messages = 0;
                  instance.client.chans[args.to.toLowerCase()].unread_mentions = 0;
                }

                // if not, increment unread message and mention counts
                instance.client.chans[args.to.toLowerCase()].unread_messages++;
                
                var re = new RegExp('\\b' + instance.client.nick.toLowerCase() + '\\b', 'g');
                if(re.test(args.text.toLowerCase())){
                  instance.client.chans[args.to.toLowerCase()].unread_mentions++;
                }
              } catch (err) {}
              // log it
